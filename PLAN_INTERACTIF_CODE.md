# Plan Interactif — Code source complet

Page React (`/plan-interactif`) du site Cosmos Angré : plan 3D interactif Three.js, recherche, filtres par catégorie, parcours consommateur et fiche détail.

---

## Fichier : `Frontend/src/pages/public/InteractiveMapPage.tsx`
...
import {
  ...
  Filter, ChevronDown, X, Eye, Route,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-1.5 h-full">
                  ...
                  {storeList.map(store => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${store.status === 'loue' ? 'bg-green-500' : store.status === 'accord' ? 'bg-amber-400' : 'bg-gray-300'}`} />
                      </div>
                    </button>
                  ))}
                  {storeList.length === 0 && (
            ...
            <div className="flex flex-col gap-1 min-h-0">
              ...
              <div className="rounded-lg overflow-hidden relative bg-transparent flex-1 min-h-0">
                ...
                {storeInfo && (
                  <div className="absolute bottom-4 right-4 z-20 w-80 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg p-5 border border-cosmos-cream animate-fade-in">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[9px] font-inter font-medium px-2 py-0.5 rounded-full ${
                            storeInfo.status === 'loue' ? 'bg-green-100 text-green-700' :
                ...
                <div className="absolute top-3 right-3 z-20">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-sm border border-cosmos-cream text-[10px] font-inter font-medium text-cosmos-night/60 hover:text-cosmos-night transition-colors"
                  >
                    <Info className="w-3 h-3" strokeWidth={1.5} />
                    {showLegend ? 'Masquer légende' : 'Afficher légende'}
                  </button>
                  {showLegend && (
                    <div className="mt-2 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-cosmos-cream p-4 w-64 animate-fade-in">
                      ...
                      <div className="space-y-1.5">
                        {[
                          { label: 'Loué', color: 'bg-green-500', count: STORE_DATA.filter(s => s[9] === 'loue').length },
                          { label: 'Accord de principe', color: 'bg-amber-400', count: STORE_DATA.filter(s => s[9] === 'accord').length },
                          { label: 'Libre / Disponible', color: 'bg-gray-300', count: STORE_DATA.filter(s => s[9] === 'libre').length },
                        ].map(s => (
...
```bash
npm install three react-i18next lucide-react
npm install -D @types/three
```

## Assets requis
- `Frontend/public/plan-cosmos.png` — image du plan 2D (3308 × 2339 px)

## Route (dans `Frontend/src/routes/index.tsx`)
```tsx
{ path: 'plan-interactif', element: wrap(<InteractiveMapPage />) },
```

## Helpers SEO importés
- `../../lib/seo/Seo` — composant `<Seo>` (title/description/keywords/jsonLd)
- `../../lib/seo/jsonLd` — fonction `breadcrumbJsonLd`

## Classes Tailwind custom utilisées
`bg-cosmos-warm`, `bg-cosmos-night`, `text-cosmos-cream`, `text-cosmos-gold`, `text-cosmos-night`, `border-cosmos-cream`, `font-cormorant`, `font-inter`, `card`, `input`, `animate-fade-in`
